...
public class SharedController {
	...
	public void updateOrCreateTask(TaskListView taskListView,SharedView sharedView, ListModel resultIdList){
		...
		if(taskListView.getItems() != null){
			for(TaskView task : taskListView.getItems()){
	...
	public void insertOrUpdateSharedEmail(SharedView sharedView,TaskModel resultTaskId,ListModel resultIdList,TaskView task,TaskListView taskListView){
		...
		if(sharedView.getEmails() != null){
			for(String email : sharedView.getEmails()){
				...
				if(findMyEmail == null){
		    		...
		    		sharedModelCreateMyEmail.setSync(1);
    ...
    public @ResponseBody List<SharedModelView> haveShared(HttpServletRequest request) {
	...
	@RequestMapping("/haveSharedFE") 
    public @ResponseBody String haveSharedFE(HttpServletRequest request) {
    	...
    	String response = gson.toJson(sharedModelViewResponse,type);
